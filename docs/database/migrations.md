...
| 59  | add_abacatepay_fields                           | Adiciona `payment_provider`, `external_payment_id`, `pix_code`, `pix_expires_at` em `financial_records`; cria tabela `webhook_processing_log` (idempotência de webhooks); index em `external_payment_id` |
| 60  | add_teacher_abacate_pay_key                     | Adiciona `abacate_pay_api_key` (criptografada) e `abacate_pay_webhook_secret` (UUID por professor) em `teachers`                                                                                         |
| 61  | grant_encrypt_functions_authenticated           | Grant de `encrypt_sensitive_data` e `decrypt_sensitive_data` para role `authenticated` (necessário para Edge Functions chamarem via `userClient`)                                                        |
| 62  | financial_summary_date_range                    | Adiciona parâmetros `p_date_from` / `p_date_to` em `get_financial_summary` — habilita filtro por período (mês/semestre/ano) nos cards financeiros do dashboard; `total_overdue` não é filtrado por data  |
| 63  | add_record_type_to_financial_records            | Adiciona coluna `record_type` (`'avulsa'` \| `'pacote'` \| `'manual'`) em `financial_records` — substitui pattern implícito de `class_log_id IS NULL` como discriminador                                 |
| 64  | class_logs_summary_period_filter                | Adiciona parâmetros `p_date_from` / `p_date_to` em `get_class_logs_summary` — filtro por período nas aulas; `NULL` preserva comportamento anterior (sem filtro)                                          |
| 65  | fix_class_date_consistency_constraint           | Corrige constraint de consistência de datas em `class_logs` (conflito entre `class_date` e `start_at`/`end_at`)                                                                                          |
| 66  | create_log_performance_stub                     | Cria stub da função de performance de logs (preparação para migration 72)                                                                                                                                |
| 67  | fix_prevent_hourly_rate_trigger_user_roles      | Corrige trigger de hourly_rate: removia referência à tabela `user_roles` (dropada em migration 45)                                                                                                       |
| 68  | fix_hourly_rate_trigger_teacher_id_comparison   | Corrige comparação de `teacher_id` no trigger de hourly_rate — cast incorreto causava NULL silencioso                                                                                                    |
| 69  | fix_activity_tenant_isolation_teacher_id        | Corrige isolamento de tenant em `activities`: RLS comparava via `auth.uid()` diretamente em vez de `profiles.teacher_id`                                                                                 |
| 70  | fix_activity_tenant_isolation_null_role         | Corrige edge case de role NULL no isolamento de tenant de atividades                                                                                                                                     |
| 71  | fix_class_logs_summary_orphan_logs              | Corrige `get_class_logs_summary` para excluir class logs órfãos (sem `teacher_id` válido) da contagem                                                                                                    |
| 72  | fix_log_performance_search_path                 | Corrige `search_path` na função de performance de logs (anti search_path hijacking, padrão das demais funções)                                                                                           |
