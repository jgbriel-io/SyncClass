    ...
    queryFn: async () => {
      ...
      return {
        activities: {
          ...
          percentage: totalActivities > 0 ? (submittedActivities / totalActivities) * 100 : 0,
        },
        classes: {
          total: totalClasses,
...
export const StudentHome = () => {
  ...
  const { streak, badges, progress } = useStudentGamification(profile?.student_id);
...
### Sprints Futuras (Não Planejadas)
- **Sprint 21:** Relatórios avançados para professores (gráficos, insights)
- **Sprint 22:** Integração com WhatsApp (lembretes de aula)
- **Sprint 23:** Multi-tenancy (suporte para múltiplas escolas)
...
- `supabase/migrations/` — padrão de triggers SQL
