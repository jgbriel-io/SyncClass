...
| Seção                         | Conteúdo                                             |
| ----------------------------- | ---------------------------------------------------- |
| 4.1 Público-Alvo              | 3 perfis + contexto de levantamento (freela + Forms) |
| 4.2 Requisitos Funcionais     | 31 RF implementados + 4 planejados (RF31-33, RF35)   |
| 4.3 Requisitos Não Funcionais | 36 RNF (100% implementados)                          |
| 4.4 Regras de Negócio         | 59 regras por módulo                                 |
| 4.5 Casos de Uso              | 26 UCs com 3 fluxos principais detalhados            |
| 4.6 Matriz de Rastreabilidade | RF → arquivo → migration → teste                     |
| 4.7 Resumo Quantitativo       | Contagens consolidadas                               |

### Citações Planejadas

- Sommerville (2011) — definição de requisitos de software
- Pressman e Maxim (2016) — engenharia de requisitos
- LGPD — Art. 16, I (para RN-043 e relacionados)

### Pendências Abertas

- [ ] Completar matriz de rastreabilidade para RF05, RF07-09, RF14-16, RF18-19, RF22-28
- [ ] RF31-33/RF35 mover para trabalhos futuros (cap10) — ainda no texto como "Futuros"
- [ ] Declarar explicitamente que RN-049 (paginação) = RNF19 (mesma regra, dois IDs) ou consolidar
- [ ] Inserir referência ao Forms (Apêndice A) e ao professor do freela após Forms ser realizado
...
Os requisitos do SyncClass foram levantados a partir de demanda real de um
professor autônomo de idiomas, junto ao qual foram realizadas sessões de
elicitação de requisitos que serviram de base para a definição do escopo e
das funcionalidades da plataforma. A plataforma foi utilizada em contexto
real por esse profissional e por seus alunos durante o período de
desenvolvimento, permitindo validar empiricamente as funcionalidades
principais. Complementarmente, foi conduzido um estudo exploratório com
participantes da área de educação para corroborar a percepção do problema
(Apêndice A). A avaliação formal de satisfação e impacto com usuários finais
está mapeada como trabalho futuro no Capítulo 10.
...
| ------ | --------------------------------------------- | ------------ | -------------- | --------------------------- |
| RF31   | Sistema de notificações (email, push, in-app) | Notificações | Média          | Trabalho futuro             |
| RF32   | Exportação de relatórios em PDF               | Relatórios   | Média          | Trabalho futuro             |
| RF33   | Integração com Google Calendar                | Integrações  | Baixa          | Trabalho futuro             |
| RF34   | Gateway de pagamento real (AbacatePay — PIX)  | Financeiro   | Média          | ✅ Implementado (Sprint 30) |
| RF35   | Sistema de gamificação (badges, conquistas)   | Gamificação  | Baixa          | Trabalho futuro             |
