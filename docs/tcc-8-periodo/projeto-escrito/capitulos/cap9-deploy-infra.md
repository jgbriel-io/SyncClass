...
| Seção                               | Conteúdo                                                                    |
| ----------------------------------- | --------------------------------------------------------------------------- |
| 9.1 Visão Geral                     | Cloudflare Pages (frontend) + Supabase BaaS (backend), sem servidor próprio |
| 9.2 Ambientes de Execução           | Dev (localhost) + Produção (main → Cloudflare Pages)                        |
| 9.3 Frontend — Cloudflare Pages     | Build Vite → CDN global, zero custo, rollback atômico                       |
| 9.4 Backend — Supabase as a Service | PostgreSQL, Auth JWT, Storage, PostgREST, 9 Edge Functions                  |
| 9.5 CI/CD — GitHub Actions          | Pipeline ci.yml (lint→type-check→test→build) + auditoria semanal            |
| 9.6 Mecanismos de Segurança         | HTTPS, RLS, rate limiting SQL, logger.ts                                    |
| 9.7 Fluxo de Deploy Atual           | push → CI → Cloudflare Pages (automático, atômico)                          |

### Citações Planejadas

- Supabase (2024) — BaaS: PostgreSQL gerenciado, Auth, Storage, Edge Functions
