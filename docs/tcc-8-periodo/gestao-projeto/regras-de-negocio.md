...
**Status:** ✅ Consolidado — 62 regras (59 originais + RN-060/061/062 AbacatePay Sprint 30)
...
- **Fluxo manual (legado):** QR Code gerado no frontend com chave PIX do professor; aluno envia comprovante para aprovação
- **Fluxo AbacatePay (Sprint 30):** QR Code gerado via API AbacatePay; webhook `billing.paid` / `pixQrCode.paid` confirma pagamento automaticamente
- `payment_provider`: `'manual'` (legado) ou `'abacate_pay'` (automático)
- **Fonte:** `create-abacate-payment/index.ts`, `abacate-webhook/index.ts`
...
## 10. AbacatePay — Pagamento PIX Automático (Sprint 30)

### RN-060: Provedor de Pagamento

- Campo `payment_provider` em `financial_records`: `'manual'` ou `'abacate_pay'`
- Fluxo manual (legado): aluno envia comprovante → professor aprova/rejeita
- Fluxo AbacatePay: QR Code gerado via API → webhook confirma automaticamente
- **Fonte:** migration AbacatePay (Sprint 30)

### RN-061: Confirmação Automática via Webhook

- Webhook recebe eventos `billing.paid` ou `pixQrCode.paid` do AbacatePay
- Status da cobrança muda para `pago` automaticamente sem ação do professor
- `paid_at` preenchido com timestamp do evento do webhook
- Idempotência garantida: reprocessamento do mesmo evento não duplica confirmação
- **Fonte:** `supabase/functions/abacate-webhook/index.ts`

### RN-062: Reembolso — Automático vs Manual

- Se `payment_provider = 'abacate_pay'`: reembolso via API AbacatePay (`refund-abacate-payment`)
- Se `payment_provider = 'manual'`: reversão manual de status pelo professor
- Status muda para `extornado` em ambos os casos
- Webhook `checkout.refunded` atualiza status automaticamente no fluxo AbacatePay
- **Fonte:** `supabase/functions/refund-abacate-payment/index.ts`
