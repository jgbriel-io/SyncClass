...
* **Commit de destaque:** `86475d8` - *feat: ibge cep api*.

### Sprint 2 — Autenticação & Usuários
**Período:** 26–29 Janeiro 2026 | **Status:** ✅ Concluída
* Criação de fluxos para alunos e professores com *roles* específicos no Supabase Auth.
* Migração de tabelas para componentes **shadcn/ui**.
* Criação de views compartilhadas (`FinancialView`, `DashboardView`).
* Padronização de containers de página e estados vazios (*EmptyStates*).
...
* **Segurança:** Implementação de Rate Limiting e proteção contra *Search Path Hijacking*.
* **Privacidade:** Criptografia de chaves PIX e triggers de invalidação de sessões.
* **Performance:** Criação de *Materialized Views* para dashboards financeiros.
...
* *Restore* integral do codebase e aplicação das migrations finais (22 e 23).
