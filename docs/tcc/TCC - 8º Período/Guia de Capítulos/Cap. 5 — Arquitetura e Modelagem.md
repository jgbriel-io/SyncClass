...
- **Performance:** O uso de *lazy loading* por rota **minimiza** o bundle inicial enviado ao cliente.
...
- **TIMESTAMPTZ:** Todos os campos de data são *timezone-aware*.
...
- **LGPD:** Inclusão de `anonymized_at` para anonimização e `is_deleted` para *soft delete*.
...
```
**Estado Atual:** Mais de 40 políticas implementadas cobrindo todas as tabelas para os 3 perfis (*roles*).
