...
export function AdminResetPasswordDialog({
  ...
  const handleSubmit = () => {
    ...
    if (!PASSWORD_REGEX.upper.test(newPassword)) { toast.error("A senha deve conter pelo menos uma letra maiúscula."); return; }
    if (!PASSWORD_REGEX.lower.test(newPassword)) { toast.error("A senha deve conter pelo menos uma letra minúscula."); return; }
    if (!PASSWORD_REGEX.number.test(newPassword)) { toast.error("A senha deve conter pelo menos um número."); return; }
    if (!PASSWORD_REGEX.special.test(newPassword)) { toast.error("A senha deve conter pelo menos um caractere especial."); return; }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          {user && (
            <DialogDescription>
              Nova senha para <strong>{user.profile?.full_name ?? user.email}</strong>.
            </DialogDescription>
          )}
        </DialogHeader>
        {user && (
