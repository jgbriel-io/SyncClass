...
export function UserDeleteDialog({
  ...
  const description = isArchivedProfile ? (
    <>
      ...
      <br />
      <br />A conta será removida do sistema. O email ficará disponível para
      reutilização. Esta ação não pode ser desfeita.
    </>
  ) : isHardDelete ? (
    <>
      A conta do usuário <strong>{displayName}</strong> será removida do sistema
      (Supabase Auth, perfil e vínculos). Esta ação não pode ser desfeita.
    </>
  ) : userIsInactive && !forceHardDelete ? (
    <>
      Tem certeza que deseja reativar o usuário <strong>{displayName}</strong>?
    ...
    <>
      Tem certeza que deseja arquivar o usuário <strong>{displayName}</strong>?
      Esta ação não remove a conta do Supabase Auth, apenas arquiva o usuário no
      painel.
    </>
  );
