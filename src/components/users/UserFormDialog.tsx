...
export function UserFormDialog({ open, onOpenChange, user, onSubmit, isLoading }: UserFormDialogProps) {
  const isEdit = !!user;
  const [selectedRole, setSelectedRole] = useState<AppRole>((user?.role?.role as AppRole) || "admin");
  ...
  const isBrazilSelected = selectedCountry.toLowerCase() === "brasil" || selectedCountry.toLowerCase() === "brazil";
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { email: user?.email || "", fullName: user?.profile?.full_name || "", name: user?.profile?.full_name || "", role: selectedRole },
  });

  useEffect(() => {
    if (selectedRole === "student" && isBrazilSelected) {
      if (!selectedState || selectedState.length !== 2) { setCities([]); return; }
      setIsLoadingCities(true);
      fetchIbgeCitiesByUf(selectedState as BrStateCode).then((result) => { setCities(result); setIsLoadingCities(false); });
    } else {
      setCities([]);
    }
  }, [selectedState, selectedRole, isBrazilSelected]);

  const resetStudentDefaults = () => {
    reset({ name: "", email: "", phone: "", state: "", city: "", birth_date: null, hourly_rate: "", pay_day: "", origin: "outro", status: "ativo", role: "student" });
    setSelectedOrigin(""); setSelectedStatus("ativo"); setSelectedCountry("Brasil"); setSelectedState("");
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      if (userRole === "admin") {
        reset({ email: user.email, fullName: user.profile?.full_name || "", role: "admin" });
      } else if (userRole === "student" && user.student) {
        const s = user.student;
        reset({ name: s.name || "", email: s.email || user.email, phone: s.phone || "", state: s.state || "", city: s.city || "", birth_date: s.birth_date ? format(new Date(s.birth_date), "dd/MM/yyyy", { locale: ptBR }) : null, hourly_rate: s.hourly_rate ? String(s.hourly_rate) : "", pay_day: s.pay_day ? String(s.pay_day) : "", origin: (s.origin as StudentOrigin) || "outro", status: (s.status as StudentStatus) || "ativo", role: "student" });
        ...
        reset({ name: t.name || "", email: t.email || user.email, phone: t.phone || "", role: "teacher" });
      }
    } else {
      if (selectedRole === "admin") reset({ email: "", fullName: "", role: "admin" });
  ...
  const handleFormSubmit = (data: FormData) => {
    if (selectedRole === "admin" && "fullName" in data) {
      ...
      const s = data as any;
      const hourlyRateNumber = s.hourly_rate ? parseFloat(s.hourly_rate.replace(/[^.\d,]/g, "").replace(",", ".")) : null;
      const normalizedPhone = s.phone ? removeCountryCode(s.phone, selectedCountry) : null;
      onSubmit({
        email: data.email, fullName: s.name, role: "student",
        studentData: { name: s.name, country: selectedCountry, state: isBrazilSelected ? (selectedState || null) : (s.state || null), city: s.city || null, phone: normalizedPhone || null, email: data.email, origin: selectedOrigin as StudentOrigin, status: selectedStatus, birth_date: s.birth_date ? brDateToIso(s.birth_date) : null, hourly_rate: hourlyRateNumber, pay_day: s.pay_day ? Number(s.pay_day) : null, teacher_id: selectedTeacherId },
      });
    } else if (selectedRole === "teacher" && "name" in data) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const t = data as any;
      const normalizedPhone = t.phone?.trim().length > 0 ? removeCountryCode(t.phone, "Brasil") : undefined;
      onSubmit({ email: data.email, fullName: t.name, role: "teacher", teacherData: { name: t.name, email: data.email, ...(normalizedPhone && { phone: normalizedPhone }) } });
    }
  };
  ...
  return (
    <BaseDialog open={open} onOpenChange={onOpenChange} title={user ? usersContent.formDialog.titleEdit : usersContent.formDialog.titleNew} size={selectedRole === "admin" ? "SM" : "MD"}>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        ...
        {selectedRole === "admin" && (
          <UserFormAdminFields register={register as any} errors={errors as any} isLoading={isLoading} isEdit={isEdit} />
        )}

        {selectedRole === "student" && (
          <UserFormStudentFields
            register={register as any} errors={errors as any} setValue={setValue as any} watch={watch as any}
            isLoading={isLoading} isEdit={isEdit}
            activeTeachers={activeTeachers} loadingTeachers={loadingTeachers}
            selectedTeacherId={selectedTeacherId} setSelectedTeacherId={setSelectedTeacherId}
            teacherError={teacherError} setTeacherError={setTeacherError}
        ...
        {selectedRole === "teacher" && (
          <UserFormTeacherFields register={register as any} errors={errors as any} setValue={setValue as any} isLoading={isLoading} isEdit={isEdit} />
        )}
