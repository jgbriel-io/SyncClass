...
export function ActivitiesTableRow({
  ...
  return (
    <tr className="group hover:bg-muted/30 transition-colors">
      ...
      <td className={`${CELL_BASE} ${STICKY_CELL} ${getXLColumnClasses()}`} style={STICKY_SHADOW}>
        <div className="flex items-center gap-4 overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium truncate" title={activity.students?.name || "—"}>{activity.students?.name || "—"}</p>
            {isAdmin ? (
              <p className="text-xs mobile:text-[11px] tablet:text-[11px] laptop:text-[11px] text-muted-foreground mt-0.5 truncate" title={activity.teachers?.name ?? "—"}>
      ...
      {/* Arquivo - M */}
      <td className={cn(CELL_BASE, "hidden sm:table-cell")} style={{ width: COL.ARQUIVO, minWidth: COL.ARQUIVO }}>
      ...
      {/* Prazo - S */}
      <td className={cn(CELL_BASE, "hidden sm:table-cell tabular-nums")} style={{ width: COL.PRAZO, minWidth: COL.PRAZO }}>
        <span className="text-muted-foreground truncate block" title={formatActivityDueDate(activity.due_date)}>{formatActivityDueDate(activity.due_date)}</span>
      </td>
      ...
      {/* Entregue em - M */}
      <td className={cn(CELL_BASE, "hidden sm:table-cell tabular-nums")} style={{ width: COL.ENTREGUE_EM, minWidth: COL.ENTREGUE_EM }}>
        <span className="text-muted-foreground truncate block" title={activity.delivered_at ? format(new Date(activity.delivered_at), "dd/MM/yyyy HH:mm", { locale: ptBR }) : "—"}>
          {activity.delivered_at ? format(new Date(activity.delivered_at), "dd/MM/yyyy HH:mm", { locale: ptBR }) : "—"}
        </span>
      </td>

      {/* Avaliar (Corrigir/Atualizar) - S */}
