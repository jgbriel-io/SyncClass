...
import {
  COL as CL_COL,
  TABLE_MIN_W as CL_TABLE_MIN_W,
} from "@/components/classes/ClassesTableRow.constants";
import { cn } from "@/lib/utils";
import { tableThLarge, tableThSmall } from "@/lib/utils/tableColumns";
          ...
          <TableHeader>
            <TableRow className="border-b bg-muted/50">
              <TableHead
                ...
                style={{ width: "1%" }}
              >
                {classesContent.table.colStatus}
              </TableHead>
              <TableHead
                ...
                style={{
                  width: CL_COL.ALUNO,
                  minWidth: CL_COL.ALUNO,
                  boxShadow: "2px 0 5px -2px rgba(0,0,0,0.1)",
                }}
              >
                {classesContent.table.colStudent}
              </TableHead>
              <TableHead
                className={cn(tableThLarge, "hidden sm:table-cell")}
                style={{
                  width: CL_COL.INFORMACOES,
                  minWidth: CL_COL.INFORMACOES,
                }}
              >
                {showTeacherColumn
                  ? classesContent.table.colInfo
                  : classesContent.table.colTitle}
              </TableHead>
              <TableHead
                className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-2 py-2 align-middle whitespace-nowrap"
                style={{ width: CL_COL.DATA, minWidth: CL_COL.DATA }}
              >
                {classesContent.table.colDate}
              </TableHead>
              <TableHead
                className={cn(tableThSmall, "hidden sm:table-cell")}
                style={{ width: CL_COL.DURACAO, minWidth: CL_COL.DURACAO }}
              >
                {classesContent.table.colDuration}
              </TableHead>
              <TableHead
                className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-2 py-2 align-middle whitespace-nowrap"
                style={{ width: CL_COL.NOTA, minWidth: CL_COL.NOTA }}
              >
                {classesContent.table.colGrade}
              </TableHead>
              <TableHead
                className={cn(tableThSmall, "hidden xl:table-cell")}
                style={{
                  width: CL_COL.FINANCEIRO,
                  minWidth: CL_COL.FINANCEIRO,
                }}
              >
              ...
              >
                {classesContent.table.colActions}
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
      ...
      {filteredLogs.length === 0 && (
        <div className="border-t">
          {logs.length === 0 ? (
            ...
            <EmptyState
              icon={Search}
              title={classesContent.table.noResults}
              message={classesContent.table.noResultsHint}
            />
          )}
        </div>
      )}
