...
/**
 * Calcula o status real de uma cobrança.
 * Status finais (pago, abonado, extornado, cancelado, validando) são lidos direto do DB.
 * "atrasado" é derivado: pendente + due_date < hoje.
 */
export function getFinancialActualStatus(record: {
  ...
  if (record.status === "validando") return "validando";
