...
describe("Design Tokens: Typography", () => {
  describe("TYPOGRAPHY", () => {
    ...
    it("deve ter os valores corretos para texto secundário", () => {
      ...
      expect(TYPOGRAPHY.SMALL_MEDIUM).toBe(
    ...
    it("deve ter o valor correto para header de tabela", () => {
      expect(TYPOGRAPHY.TABLE_HEADER).toBe(
  ...
  describe("typographyWithColor()", () => {
    it("deve combinar variante com cor", () => {
      expect(typographyWithColor("BODY", "MUTED")).toBe(
        "text-sm text-muted-foreground"
      );
      expect(typographyWithColor("CAPTION", "DESTRUCTIVE")).toBe(
        "text-xs text-destructive"
      );
      expect(typographyWithColor("H1", "PRIMARY")).toBe(
  ...
  describe("customTypography()", () => {
    ...
    it("deve criar classe com tamanho, peso e cor", () => {
      expect(customTypography("sm", "MEDIUM", "PRIMARY")).toBe(
        "text-sm font-medium text-primary"
      );
      expect(customTypography("xs", "SEMIBOLD", "DESTRUCTIVE")).toBe(
        "text-xs font-semibold text-destructive"
      );
    });

    it("deve criar classe com tamanho e cor (sem peso)", () => {
      expect(customTypography("sm", undefined, "MUTED")).toBe(
        "text-sm text-muted-foreground"
      );
    });
  });
  ...
  describe("Hierarquia de tamanhos", () => {
    it("DISPLAY deve ser maior que H1", () => {
      expect(TYPOGRAPHY_SIZES_PX.DISPLAY).toBeGreaterThan(
        TYPOGRAPHY_SIZES_PX.H1
      );
    });
    ...
    it("BODY deve ser maior que CAPTION", () => {
      expect(TYPOGRAPHY_SIZES_PX.BODY).toBeGreaterThan(
        TYPOGRAPHY_SIZES_PX.CAPTION
      );
    });
  });
