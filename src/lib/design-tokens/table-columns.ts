...
export const CELL_BASE =
  "px-2 py-2 mobile:px-2 mobile:py-2 tablet:px-2 tablet:py-2 laptop:px-2 laptop:py-2 align-middle text-left text-xs whitespace-nowrap";

/** Classe para célula sticky (primeira coluna) */
export const STICKY_CELL =
  "tablet:sticky tablet:left-0 z-20 bg-card group-hover:bg-muted transition-colors";

/** Sombra para célula sticky */
export const STICKY_SHADOW = { boxShadow: "2px 0 5px -2px rgba(0,0,0,0.1)" };

/** Classe para header sticky */
export const STICKY_HEADER = "tablet:sticky tablet:left-0 z-30 bg-muted";

/** Classe base para TableHead */
export const TABLE_HEAD_BASE =
  "text-left text-xs font-medium text-muted-foreground uppercase tracking-wider px-2 py-2 align-middle whitespace-nowrap";
...
export function getColumnStyle(
  size: keyof typeof TABLE_COLUMN_SIZES,
  responsive = false
) {
 ...
 */
export function calculateTableMinWidth(
  columns: (keyof typeof TABLE_COLUMN_SIZES)[]
): number {
